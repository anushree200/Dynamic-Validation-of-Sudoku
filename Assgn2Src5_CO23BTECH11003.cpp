...
atomic<bool> sudokuInvalid(false); // Global flag for early termination
ofstream outFile("outputtasearly.txt");
...
void *workerfunction(void *arg)
{
    ...
    while (true)
    {
        ...
        if (sudokuInvalid.load(memory_order_relaxed))
        {
            break; // Terminate if Sudoku is invalid
        }
        ...
        // If the task is invalid, set the global flag
        if (!valid)
        {
            sudokuInvalid.store(true, memory_order_relaxed);
        }
        ...
        results.emplace_back(task, valid ? "valid" : "invalid");

        // Record times for entering and exiting CS
        enterTimes.push_back(duration_cast<microseconds>(enterTime - requestTime));
        exitTimes.push_back(duration_cast<nanoseconds>(leaveTime - enterTime));
    }
...
int main()
{
    ...
    {
        threadIds[i] = i;
        pthread_create(&threads[i], NULL, workerfunction, (void *)&threadIds[i]);
    }
